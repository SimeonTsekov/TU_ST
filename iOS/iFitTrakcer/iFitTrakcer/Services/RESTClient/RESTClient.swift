...
class RESTClient {
    ...
    public func call<T: Endpoint>(_ endpoint: T, completion completionHandler: @escaping (Result<T.ResponseModel, Error>) -> Void) {
        ...
        session.dataTask(with: request) { [weak self] data, response, error in
            guard let self = self else {
                completionHandler(.failure(NSError(domain: "", code: -1111, userInfo: ["Message": "self is nil"])))
                return
            }
    ...
    private func validateResponse(data: Data?, response: URLResponse?, error: Error?) -> Result<Data, Error> {
        guard let httpResponse = response as? HTTPURLResponse else {
            return .failure(NSError(domain: baseURL.absoluteString, code: 400, userInfo: ["Message": "Response is not HTTPURLResponse"]))
        }

        guard 200...399 ~= httpResponse.statusCode else {
            return .failure(NSError(domain: baseURL.absoluteString, code: httpResponse.statusCode, userInfo: ["Message": "Server Error"]))
        }

        guard let data = data else {
            return .failure(NSError(domain: baseURL.absoluteString, code: httpResponse.statusCode, userInfo: ["Message": "Missing Data"]))
        }
        return .success(data)
    }
}
